        ...
        if insect.is_ant():
            ...
            if self.ant != None:
                if self.ant.container == True and insect.container == True:
                    assert self.ant.container != True, 'BodyguardAnt cannot contain BodyguardAnt'.format(insect, self)
                elif self.ant.container == True:
                    assert self.ant.has_ant(), 'cannot add third ant'.format(insect,self) 
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.queen_count >= 2:    
            ...
            cur_place = self.place
            while  cur_place.exit != None:
                if cur_place.ant != None and cur_place.ant.name != 'Queen':
                    if cur_place.ant.doubled_damage == False:
                        cur_place.ant.damage = cur_place.ant.damage * 2
                        cur_place.ant.doubled_damage = True
                        if cur_place.ant.container == True:
                            if cur_place.ant.ant.name != 'Queen':
                                cur_place.ant.ant.damage = cur_place.ant.ant.damage *2
                                cur_place.ant.ant.doubled_damage = True
                cur_place = cur_place.exit
            cur_place = self.place
            while  cur_place.entrance != None:
                if cur_place.ant != None and cur_place.ant.name != 'Queen':
                    if cur_place.ant.doubled_damage == False:
                        cur_place.ant.damage = cur_place.ant.damage * 2
                        cur_place.ant.doubled_damage = True
        ...
        colony.queen = QueenPlace(end_place , self.place)
