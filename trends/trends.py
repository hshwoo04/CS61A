...
# Name: Sung Hyun Woo, Hector Gonzalez
...
def analyze_tweet_sentiment(tweet):
    ...
    """remove all none and where sentiment is left
    if remove all non and the length is zero"""

    words=tweet_words(tweet)
    sentiment_sum=0
    filter_words=list(filter(has_sentiment, map(get_word_sentiment, words)))
    if len(filter_words)==0:
        return make_sentiment(None)
    else:
        words_list=list(map(sentiment_value, filter_words))
        for word in words_list:
            sentiment_sum+=word
        average=sentiment_sum/len(words_list)
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    poly_x = list(map(lambda x: latitude(x), polygon)) # [1, 3, 5, 1]
    poly_y = list(map(lambda x: longitude(x), polygon)) # [2, 4, 0, 2]
    
    area, x_coordinate, y_coordinate = 0, 0, 0
    for num in range(0, len(polygon)-1):
        area = area + (((poly_x[num])*(poly_y[num+1]))-((poly_x[num+1])*(poly_y[num])))
        x_coordinate = x_coordinate + ((poly_x[num] + poly_x[num+1])*((poly_x[num]*poly_y[num+1])-(poly_x[num+1]*poly_y[num])))
        y_coordinate = y_coordinate + ((poly_y[num] + poly_y[num+1])*((poly_x[num]*poly_y[num+1])-(poly_x[num+1]*poly_y[num])))
    area = area * .5
    
    if area != 0:
        x_coordinate= (1/(6*area))*x_coordinate
        y_coordinate= (1/(6*area))*y_coordinate
        return(x_coordinate, y_coordinate, abs(area))
    else:
        return(poly_x[0]/1, poly_y[0]/1, 0.0)
...
def find_state_center(polygons):
    ...
    -156.21763
    """
    poly_map = list(map(find_centroid, polygons))
    poly_x = list(map(lambda x: x[0], poly_map))
    poly_y = list(map(lambda x: x[1], poly_map))
    poly_A = list(map(lambda x: x[2], poly_map))
    if len(poly_map) == 1:
        return make_position(poly_x[0], poly_y[0])
    else:
        x_coordinate, y_coordinate = 0, 0 
        for num in range(0, len(poly_map)):
            x_coordinate= x_coordinate + poly_x[num]*poly_A[num]
            y_coordinate = y_coordinate + poly_y[num]*poly_A[num]
        A_sum = sum(poly_A)
        x_coordinate= x_coordinate/A_sum
        y_coordinate= y_coordinate/A_sum
...
def group_tweets_by_state(tweets):
    ...
    def nearest_state(tweet):
        ...
        for state in us_states:
            ...
            if geo_distance(centroid,tweet_location(tweet))<shortest_distance:
                shortest_distance=geo_distance(centroid,tweet_location(tweet))
                tweet_state=state
        return tweet_state

    for tweet in tweets:
        tweet_state=nearest_state(tweet)
        value=[]
        if tweets_by_state.get(tweet_state,None) !=None:
            value=tweets_by_state.get(tweet_state,None)
            value.append(tweet)
        else:
            value.append(tweet)
        tweets_by_state[tweet_state]=value
...
def average_sentiments(tweets_by_state):
    ...
    for tweet_state in tweets_by_state:
        sum_sentiment=None
        num=0
        for tweet in tweets_by_state[tweet_state]:
            sentiment=analyze_tweet_sentiment(tweet)
            if has_sentiment(sentiment)==True:
                if sum_sentiment==None:
                    sum_sentiment=0
                sum_sentiment+=sentiment_value(sentiment)
                num+=1
        if sum_sentiment!=None:
            averaged_state_sentiments[tweet_state]=sum_sentiment/num
