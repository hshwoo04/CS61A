...
(define (list-partitions total max-pieces max-value)
  (define (helper total max-pieces max-value curlist)
    (cond ((= total 0) (list curlist))
          ((< total 0) nil)
          ((= max-pieces 0) nil)
          ((< max-value 1) nil)
          (else (append (helper (- total max-value) (- max-pieces 1) max-value (append curlist (list max-value)))
              (helper total max-pieces (- max-value 1) curlist)))))
...
(define (tree-sums tree)
  (cond 
    ...
    (else (define (addfn val) (+ (entry tree) val)) (map addfn (flatten (map tree-sums (children tree)))))))

(define (flatten curlists)
  (cond
    ((null? curlists) nil)
    ((list? (car curlists)) (append (flatten (car curlists))(flatten (cdr curlists))))
    (else (cons (car curlists) (flatten (cdr curlists))))))
