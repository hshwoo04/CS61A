...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("Too many or too few varibles")
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(fn):
            arguments = Pair(target.second, vals.second)
            lam = Pair(lambda_sym, arguments)
            env.define(fn, scheme_eval(lam,env))
        else:
            raise SchemeError( str(fn)+ "is not a symbol")
        return (fn, None)
...
def do_cond_form(vals, env): # Do not change, Q18 will not work if changed
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            elif len(clause.second) >= 2:
                return do_begin_form(clause.second, env)
            else:
                return (quote(test), env)
